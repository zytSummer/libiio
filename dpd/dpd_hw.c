...
static int32_t hw_mem_write(const void *base, uint32_t offset, uint32_t data)
{
    ...
    *(uint32_t *)((uintptr_t)base + offset) = data;

    return 0;
}

static int32_t hw_mem_read(const void *base, uint32_t offset, uint32_t *data)
{
    if (s_g_dpd_hw.init_flag != DPD_HW_ININED)
    {
        printf("Warning!! dpd module has been initialed!\n");
    ...
    *data = *(uint32_t *)((uintptr_t)base + offset);
...
uint8_t dpd_luts_access_test(void)
{
	...
	{
		hw_mem_write(s_g_dpd_hw.dpd_ctrl_addr, entry_addr[i], wr_data[i]);

		hw_mem_read(s_g_dpd_hw.dpd_ctrl_addr, entry_addr[i], &rd_data[i]);
	}
    ...
    if(errCode == 0)
    {
        ...
        for(int i = 0; i < 5; i++)
        {
            hw_mem_write(s_g_dpd_hw.dpd_mem_addr, i*4, 0x11111111*(i+1));
            hw_mem_read(s_g_dpd_hw.dpd_mem_addr, i*4, &rd_data[i]);
#if 0
    ...
    if(errCode == 0)
    {
        ...
        for(int i = 0; i < 5; i++)
        {
            hw_mem_write(s_g_dpd_hw.dpd_mem_addr, end-(i+1)*4, 0x11111111*(i+1));
            hw_mem_read(s_g_dpd_hw.dpd_mem_addr, end-(i+1)*4, &rd_data[i]);
...
uint8_t dpd_luts_write(uint8_t lutId, uint32_t *pLut)
{
    ...
    if(errCode == 0)
    {
        ...
        {
            hw_mem_write(s_g_dpd_hw.dpd_mem_addr, i*4, pLut[i]);
        }
    }
...
uint8_t dpd_luts_read(uint8_t lutId, uint32_t *pLut)
{
    ...
    if(errCode == 0)
    {
        for(int i = 0; i < DPD_LUT_DEPTH; i++)
        {
            hw_mem_read(s_g_dpd_hw.dpd_mem_addr, i*4, &(pLut[i]));
        }
    }
...
uint32_t dpd_read_ipVersion(void)
{
    ...
    hw_mem_read(s_g_dpd_hw.dpd_ctrl_addr, ADDR_IP_VERSION, &ipVersion);
    return ipVersion;
}
...
uint64_t dpd_read_idMask(void)
{
    ...
    hw_mem_read(s_g_dpd_hw.dpd_ctrl_addr, ADDR_ID_MASK_LOW, &idMask_low);
    hw_mem_read(s_g_dpd_hw.dpd_ctrl_addr, ADDR_ID_MASK_HIGH, &idMask_high);
    return ((uint64_t)idMask_high << 32u) | idMask_low;
}

uint32_t dpd_write_scratch_reg(uint32_t scratch)
{
    hw_mem_write(s_g_dpd_hw.dpd_ctrl_addr, ADDR_SCRATCH, scratch);
...
{
    uint32_t scratch = 0;
    hw_mem_read(s_g_dpd_hw.dpd_ctrl_addr, ADDR_SCRATCH, &scratch);
    return scratch;
}

uint8_t dpd_write_act_out_sel(Dpd_ActOut_Sel sel)
{
	hw_mem_write(s_g_dpd_hw.dpd_ctrl_addr, ADDR_ACT_OUT_SEL, (uint32_t)sel);
...
{
    uint32_t outSel;
    hw_mem_read(s_g_dpd_hw.dpd_ctrl_addr, ADDR_ACT_OUT_SEL, &outSel);
...
uint64_t dpd_write_lutid(uint64_t lutid)
{
	...
	hw_mem_write(s_g_dpd_hw.dpd_ctrl_addr, ADDR_LUTID_H, lutid_high);
	hw_mem_write(s_g_dpd_hw.dpd_ctrl_addr, ADDR_LUTID_L, lutid_low);
...
{
	uint32_t lutid_low, lutid_high;
    hw_mem_read(s_g_dpd_hw.dpd_ctrl_addr, ADDR_LUTID_L, &lutid_low);
    hw_mem_read(s_g_dpd_hw.dpd_ctrl_addr, ADDR_LUTID_H, &lutid_high);
...
{
    offset = offset / 4 * 4;
    hw_mem_write(s_g_dpd_hw.dpd_ctrl_addr, offset, value);
...
uint32_t dpd_register_read(uint8_t offset)
{
    uint32_t regVal = 0;
    offset = offset / 4 * 4;
    hw_mem_read(s_g_dpd_hw.dpd_ctrl_addr, offset, &regVal);
    return regVal;
}
...
{
	uint8_t err = 0u;
	void *base = (position == 0u) ? s_g_dpd_hw.dpd_cap0_addr : s_g_dpd_hw.dpd_cap1_addr;
	uint32_t offset = 0u;
...
uint32_t dpd_hw_open(void)
{
    ...
    s_g_dpd_hw.dpd_ctrl_addr = mmap(NULL, DPD_CTRL_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, \
                                    s_g_dpd_hw.mem_fd, DPD_CTRL_BASEADDR & ~DPD_CTRL_SIZE);
    if (s_g_dpd_hw.dpd_ctrl_addr == (void *) -1)
    {
    ...
    s_g_dpd_hw.dpd_mem_addr = mmap(NULL, DPD_MEM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, \
                                    s_g_dpd_hw.mem_fd, DPD_MEM_BASEADDR & ~DPD_MEM_SIZE);
    if (s_g_dpd_hw.dpd_mem_addr == (void *) -1)
    {
        printf("ERROR!! memory map failed at physical address 0x%x\n", DPD_MEM_BASEADDR);
        ret = DPD_HW_MEM_MAP_ERROR;
        close(s_g_dpd_hw.mem_fd);
    ...
    s_g_dpd_hw.dpd_cap0_addr = mmap(NULL, DPD_CAP0_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, \
                                    s_g_dpd_hw.mem_fd, DPD_CAP0_BASEADDR & ~DPD_CAP0_SIZE);
    if (s_g_dpd_hw.dpd_cap0_addr == (void *) -1)
    {
        printf("ERROR!! memory map failed at physical address 0x%x\n", DPD_CAP0_BASEADDR);
        ret = DPD_HW_MEM_MAP_ERROR;
        close(s_g_dpd_hw.mem_fd);
    ...
    s_g_dpd_hw.dpd_cap1_addr = mmap(NULL, DPD_CAP1_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, \
                                    s_g_dpd_hw.mem_fd, DPD_CAP1_BASEADDR & ~DPD_CAP1_SIZE);
    if (s_g_dpd_hw.dpd_cap1_addr == (void *) -1)
    {
        printf("ERROR!! memory map failed at physical address 0x%x\n", DPD_CAP1_BASEADDR);
        ret = DPD_HW_MEM_MAP_ERROR;
        close(s_g_dpd_hw.mem_fd);
